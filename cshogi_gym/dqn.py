...
def make_position_features(env, features):
    ...
    # 繰り返し数
    features[42] = (env.repetition_hash[board.zobrist_hash()] - 1) / 3
...
def epsilon_greedy(state, legal_labels, legal_moves):
...
temperature = 0.6
def softmax(state, legal_labels, legal_moves):
    with torch.no_grad():
        q = policy_net(state)
...
def select_action(board, state):
    ...
    select, score = epsilon_greedy(state, legal_labels, legal_moves)
    #select, score = softmax(state, legal_labels, legal_moves)
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if not done:
            ...
            next_actions = [make_output_label(move, env.board.turn) for move in env.board.legal_moves]
        else:
            next_state = None
            next_actions = None
